		...
		<div class="loginContent">
			...
			<ul class="login-list">
				<li class="account-input">
					<a-icon type="user" />
					<input type="text" placeholder="请输入用户名" @focus="blurEvent" v-model="account" @keyup.enter="enterLogin" />
				</li>
				<li class="password-input">
					<a-icon type="lock" />
					<input type="password" placeholder="请输入密码" @focus="blurEvent" v-model="password" @keyup.enter="enterLogin" />
				</li>
                <li>
					<a-checkbox :checked="isRemember" @change="remember">记住账号</a-checkbox>
                </li>
				<li>
					<a-button type="primary" @click="login">登录</a-button>
				</li>
				<li><span style="color: red">{{ loginError }}</span></li>
			</ul>
		</div>
	</div>
